...
export function taskRenderCall(args: Record<string, unknown>, theme: Theme): ReturnType<typeof Text> {
	...
	switch (action) {
		...
		case "add_note":
			if (args.id !== undefined) text += ` ${theme.fg("accent", `#${args.id}`)}`;
			if (args.text) {
				const noteText = (args.text as string).length > 40
					? (args.text as string).slice(0, 40) + "…"
					: args.text as string;
		...
		case "bulk_delete":
			if (args.ids && Array.isArray(args.ids))
				text += ` ${theme.fg("dim", `(${args.ids.length} tasks)`)} ${theme.fg("accent", (args.ids as number[]).map((id: number) => `#${id}`).join(", "))}`;
			break;
		...
		case "bulk_update":
			if (args.ids && Array.isArray(args.ids))
				text += ` ${theme.fg("dim", `(${args.ids.length} tasks)`)}`;
			{
				const bulkFields = ["priority", "tags", "assignee", "estimatedMinutes"]
					.filter((f) => args[f] !== undefined);
		...
		case "remove_dependency":
			if (args.id !== undefined) text += ` ${theme.fg("accent", `#${args.id}`)}`;
			if (args.parentId !== undefined) text += ` ${theme.fg("dim", "depends on")} ${theme.fg("accent", `#${args.parentId}`)}`;
			break;
