/**
 * Dependency Operations — add, remove, check, cycle detection
 */
...
export function detectDependencyCycle(store: TaskStore, fromId: number, toId: number): boolean {
...
export function handleAddDependency(store: TaskStore, params: TaskActionParams): TaskToolResult {
	...
	if (params.parentId === undefined) {
		// Reuse parentId param as "depends on target"
		return error(store, "add_dependency", "Target task ID (parentId field) is required — the task that this task depends on");
	}
	...
	if (detectDependencyCycle(store, params.id, params.parentId)) {
		return error(store, "add_dependency", `Circular dependency: #${params.parentId} already depends on #${params.id} (directly or indirectly)`);
	}
...
export function handleRemoveDependency(store: TaskStore, params: TaskActionParams): TaskToolResult {
	...
	return result(store, "remove_dependency", `Removed dependency: #${params.id} no longer depends on #${params.parentId}`);
}
