/**
 * /import Command — Import tasks from Markdown file
 *
 * Usage:
 *   /import TASKS.md                  → auto-detect format, merge mode
 *   /import --format checklist todo.md → force checklist format
...
import { parseMarkdownTasks, type ImportFormat } from "../import/parser.js";
...
export function registerImportCommand(
	...
	pi.registerCommand("task-import", {
		...
		handler: async (args: string | undefined, ctx: ExtensionContext) => {
			...
			if (!args?.trim()) {
				ctx.ui.notify("Usage: /import [--format <format>] <path>", "warning");
				return;
			}
			...
			if (replaceMode) {
				...
				onMutate();
				ctx.ui.notify(
					`Replaced with ${result.created} tasks from ${filePath}`,
					"info",
				);
			} else {
