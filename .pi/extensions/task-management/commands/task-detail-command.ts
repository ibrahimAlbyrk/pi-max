...
class TaskDetailOverlay {
	...
	render(width: number): string[] {
		...
		// ── Status & Priority (title is in the top border, not repeated here) ──
		contentLines.push(this.padInner("", innerWidth)); // blank line after border
		const statusColor = t.status === "done" ? "success"
		...
		if (t.tags.length > 0) {
			contentLines.push(this.padInner(
				`  ${th.fg("muted", "Tags:")}     ${th.fg("accent", t.tags.join(", "))}`,
				innerWidth,
			));
		}
		...
		if (t.estimatedMinutes !== null || t.actualMinutes !== null) {
			let line = "  ";
			if (t.estimatedMinutes !== null) line += `${th.fg("muted", "Est:")} ${th.fg("dim", `${t.estimatedMinutes}m`)}  `;
		...
		// Footer (last 3 lines of allLines)
...
export async function showTaskDetailOverlay(task: Task, store: TaskStore, ctx: ExtensionContext): Promise<"back" | "close"> {
	return await ctx.ui.custom<"back" | "close">(
		(_tui, theme, _kb, done) => {
...
export function registerTaskDetailCommand(
	pi: any,
	getStore: () => TaskStore,
) {
