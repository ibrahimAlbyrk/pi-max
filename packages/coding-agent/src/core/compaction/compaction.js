...
import { computeFileLists, createFileOps, extractFileOpsFromMessage, formatFileOperations, getSummarizationSystemPrompt, serializeConversation, } from "./utils.js";
/**
...
// compaction/summarize, compaction/summarize-update
/**
...
export async function generateSummary(currentMessages, model, reserveTokens, apiKey, signal, customInstructions, previousSummary) {
    ...
    let basePrompt = previousSummary
        ? getPromptRegistry().render("compaction/summarize-update")
        : getPromptRegistry().render("compaction/summarize");
    ...
    const response = await completeSimple(model, { systemPrompt: getSummarizationSystemPrompt(), messages: summarizationMessages }, { maxTokens, signal, apiKey, reasoning: "high" });
...
// ============================================================================
// Prompt constant removed - now served from prompt registry: compaction/turn-prefix
/**
...
async function generateTurnPrefixSummary(messages, model, reserveTokens, apiKey, signal) {
    ...
    const conversationText = serializeConversation(llmMessages);
    const turnPrefixPrompt = getPromptRegistry().render("compaction/turn-prefix");
    ...
    const response = await completeSimple(model, { systemPrompt: getSummarizationSystemPrompt(), messages: summarizationMessages }, { maxTokens, signal, apiKey });
    if (response.stopReason === "error") {
