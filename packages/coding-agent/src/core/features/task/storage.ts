 ...
 *
 * Atomic writes: all file writes go through a temp file + rename pattern
 * to prevent partial writes from corrupting data on crash.
 */

import { existsSync, mkdirSync, readdirSync, readFileSync, renameSync, unlinkSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { createDefaultStore, recalculateNextIds } from "./store.js";
import type { Sprint, Task, TaskGroup, TaskIndex, TaskStorage, TaskStore } from "./types.js";
...
export class PerFileTaskStorage implements TaskStorage {
	...
	load(): TaskStore {
		...
		try {
			...
			console.error(`[task] Failed to load per-file store: ${err}`);
			return createDefaultStore();
		}
	}
		...
		if (existsSync(this.tasksDir)) {
			...
			for (const file of files) {
				try {
					...
					console.error(`[task] Failed to read task file ${file}: ${err}`);
				}
			}
		}
		...
		if (existsSync(this.groupsDir)) {
			...
			for (const file of files) {
				try {
					...
					console.error(`[task] Failed to read group file ${file}: ${err}`);
				}
			}
		}
		...
		if (existsSync(this.sprintsDir)) {
			...
			for (const file of files) {
				try {
					...
					console.error(`[task] Failed to read sprint file ${file}: ${err}`);
				}
			}
		}
	...
	private migrateParentIdToGroupId(store: TaskStore): void {
		...
		const tasksWithParentId = store.tasks.filter((t) => (t as Task & { parentId?: number }).parentId != null);
		if (tasksWithParentId.length === 0) return;

		console.error("[task] Migrating parentId hierarchy to task groups...");

		// Find unique parent task IDs
		const parentIds = new Set(tasksWithParentId.map((t) => (t as Task & { parentId: number }).parentId));
		...
		for (const parentId of parentIds) {
			...
			for (const child of store.tasks) {
				if ((child as Task & { parentId?: number }).parentId === parentId) {
					child.groupId = group.id;
					delete (child as Task & { parentId?: number }).parentId;
				}
			}
		...
		for (const task of store.tasks) {
			const mutableTask = task as Task & { parentId?: number };
			if (mutableTask.parentId !== undefined) {
				if (task.groupId === undefined || task.groupId === null) {
					task.groupId = null;
				}
				delete mutableTask.parentId;
			}
		}

		// Persist migrated state
		this.save(store);
		console.error(`[task] Migration complete: ${parentIds.size} parent task(s) converted to groups.`);
	}
	...
	save(store: TaskStore): void {
		try {
			...
			console.error(`[task] Failed to save store: ${err}`);
		}
	}
	...
	saveTask(task: Task, store: TaskStore): void {
		try {
			...
			console.error(`[task] Failed to save task #${task.id}: ${err}`);
		}
	}
	...
	saveGroup(group: TaskGroup, store: TaskStore): void {
		try {
			...
			console.error(`[task] Failed to save group #G${group.id}: ${err}`);
		}
	}
	...
	deleteGroup(id: number, store: TaskStore): void {
		try {
			...
			console.error(`[task] Failed to delete group #G${id}: ${err}`);
		}
	}
	...
	saveSprint(sprint: Sprint, store: TaskStore): void {
		try {
			...
			console.error(`[task] Failed to save sprint #S${sprint.id}: ${err}`);
		}
	}
	...
	deleteTask(id: number, store: TaskStore): void {
		try {
			...
			console.error(`[task] Failed to delete task #${id}: ${err}`);
		}
	}
	...
	saveIndex(store: TaskStore): void {
		try {
			...
			console.error(`[task] Failed to save index: ${err}`);
		}
	}
	...
	archiveTasks(tasks: Task[], store: TaskStore): void {
		...
		try {
			...
			console.error(`[task] Failed to archive tasks: ${err}`);
		}
	}
	...
	archiveSprints(sprints: Sprint[], store: TaskStore): void {
		...
		try {
			...
			console.error(`[task] Failed to archive sprints: ${err}`);
		}
	}
		...
		try {
			...
			for (const file of files) {
				try {
					...
					// Skip corrupt files — do not interrupt full load
				}
			}
			tasks.sort((a, b) => a.id - b.id);
		} catch {
			// Directory read failure — return empty list
		}
		return tasks;
	}
	...
	private needsMigration(): boolean {
		// Old format exists AND new per-file format doesn't exist yet
		return existsSync(this.oldFormatPath) && !existsSync(this.indexPath);
	}

	private migrateFromOldFormat(): TaskStore {
		console.error("[task] Migrating from tasks.json to per-file format...");

		try {
			const raw = readFileSync(this.oldFormatPath, "utf-8");
			const data = JSON.parse(raw) as TaskStore & { groups?: TaskGroup[]; nextGroupId?: number };
			...
			try {
				...
				console.error(`[task] Migration complete. Old file backed up to ${backupPath}`);
			} catch {
				console.error("[task] Migration complete. Could not rename old file.");
			}
	 ...
	 * rename() is atomic on POSIX filesystems — prevents partial writes on crash.
	 */
