 ...
 *
 * Implements the Component & Focusable interfaces from @mariozechner/pi-tui.
 */

import type { Focusable } from "@mariozechner/pi-tui";
import { matchesKey, truncateToWidth, visibleWidth } from "@mariozechner/pi-tui";
import type { Theme } from "../../../../modes/interactive/theme/theme.js";
import { PRIORITY_COLORS, priorityLabel, STATUS_ICONS } from "../rendering/icons.js";
import { findGroup, findTask as findTaskInStore, formatElapsed } from "../store.js";
import type { Task, TaskPriority, TaskStatus, TaskStore } from "../types.js";
import { PRIORITY_ORDER } from "./helpers.js";

// ─── Types ───────────────────────────────────────────────────────

/** Result emitted only when the overlay should CLOSE (detail or escape). */
export type KanbanResult = { type: "detail"; taskId: number } | null;
...
export type KanbanMutateCallback = (
	action:
		| { type: "move"; task: Task; oldStatus: TaskStatus; newStatus: TaskStatus }
...
export class KanbanBoard implements Focusable {
	/** Set by TUI when focus changes (Focusable interface). */
	...
	handleInput(data: string): void {
		...
		if (matchesKey(data, "tab")) {
			...
			this.activeRow = Math.min(this.activeRow, Math.max(0, (visCols[this.activeCol]?.tasks.length ?? 1) - 1));
			this.scrollOffset = 0;
			this.invalidate();
			return;
		}
	...
	private moveSelectedTask(direction: "left" | "right", cols: KanbanColumn[]): void {
		...
		// Find and mutate the actual task in the store (not the copy with depth)
		...
		if (newStatus === "done") {
			...
			if (storeTask.startedAt) {
				storeTask.actualMinutes = Math.round((Date.now() - new Date(storeTask.startedAt).getTime()) / 60000);
			}
	...
	render(width: number): string[] {
		...
		lines.push(truncateToWidth(th.fg("borderMuted", "│") + titlePadded + th.fg("borderMuted", "│"), width));
		...
		for (let c = 0; c < cols.length; c++) {
			...
			headerLine += isActive ? th.fg("accent", th.bold(padded)) : th.fg("muted", padded);
			if (c < cols.length - 1) {
		...
		// Dynamic maxVisibleRows based on terminal height.
		...
		const chromeLines = 9;
		// Each card row = 3 lines + 1 separator = 4 lines
		...
		if (maxRows === 0) {
			...
			lines.push(truncateToWidth(th.fg("borderMuted", "│") + emptyInner + th.fg("borderMuted", "│"), width));
		}
		...
		lines.push(truncateToWidth(th.fg("borderMuted", "│") + footerPadded + th.fg("borderMuted", "│"), width));
	...
	private renderCard(
		...
		const pri = th.fg(
			PRIORITY_COLORS[task.priority] as Parameters<Theme["fg"]>[0],
		...
		const titleColored = isSelected
			? th.fg("accent", titleTruncated)
			: task.status === "done"
				? th.fg("dim", titleTruncated)
