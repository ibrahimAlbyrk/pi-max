...
import type { Task, TaskToolDetails } from "../types.js";
import { formatElapsed } from "../ui/helpers.js";
import { PRIORITY_COLORS, priorityLabel, STATUS_ICONS } from "./icons.js";
...
export function taskRenderResult(
	...
	switch (action) {
		case "create": {
			...
			return new Text(
				theme.fg("success", "✓ Created ") +
					...
					" " +
					theme.fg(PRIORITY_COLORS[task.priority] as ThemeColor, `[${priorityLabel(task.priority)}]`) +
					(task.tags.length > 0 ? ` ${theme.fg("dim", `[${task.tags.join(", ")}]`)}` : ""),
				0,
				0,
			);
		}
		...
		case "list": {
			...
			if (!options.expanded) {
				return new Text(theme.fg("muted", `${tasks.length} task(s) (${countStr})`), 0, 0);
			}
		...
		case "update": {
			return new Text(theme.fg("success", "✓ ") + theme.fg("muted", rawText), 0, 0);
		}
...
function renderTaskLine(t: Task, theme: Theme): string {
	...
	const pri = theme.fg(PRIORITY_COLORS[t.priority] as ThemeColor, `[${priorityLabel(t.priority)}]`);
	const title = t.status === "done" ? theme.fg("dim", t.title) : theme.fg("text", t.title);
