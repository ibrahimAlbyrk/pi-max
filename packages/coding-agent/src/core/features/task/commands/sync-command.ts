...
import { syncPullContent, syncPush } from "../sync/file-sync.js";
	...
	pi.registerCommand("sync", {
		...
		handler: async (args, ctx) => {
			...
			if (action === "push" || action === "") {
				...
				ctx.ui.notify(`Synced ${store.tasks.length} tasks → ${config.path} (${config.format})`, "info");
				return;
			}
			...
			if (action === "pull") {
				...
				onMutate();
				ctx.ui.notify(`Pulled: ${result.created} new, ${result.updated} updated`, "info");
				return;
			}
