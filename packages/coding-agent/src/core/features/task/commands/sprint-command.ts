...
import type { Sprint, TaskStore } from "../types.js";
import { showTaskListOverlay } from "./tasks-command.js";
...
class SprintDashboard {
	...
	render(width: number): string[] {
		...
		contentLines.push(
			this.padInner(
				` ${th.fg("muted", "Status:")}  ${th.fg("accent", s.status)}  ${th.fg("muted", "|  Started:")}  ${th.fg("dim", s.startDate ? fmtDate(s.startDate) : "—")}`,
				innerWidth,
			),
		);
		contentLines.push(
			this.padInner(
				` ${th.fg("muted", "Progress:")} ${bar} ${th.fg("text", `${done}/${total}`)} ${th.fg("dim", `(${pct}%)`)}`,
				innerWidth,
			),
		);
		contentLines.push(this.padInner("", innerWidth));
		...
		if (doneTasks.length > 0) {
			...
			contentLines.push(
				this.padInner(
					` ${th.fg("muted", "Avg time/task:")} ${th.fg("text", formatElapsed(avgMin * 60000))}`,
					innerWidth,
				),
			);
			if (total - done > 0) {
				...
				contentLines.push(
					this.padInner(
						` ${th.fg("muted", "ETA remaining:")} ${th.fg("accent", `~${formatElapsed(etaMin * 60000)}`)}`,
						innerWidth,
					),
				);
			}
		...
		if (tasks.length > 0) {
			...
			for (const t of tasks) {
				...
				contentLines.push(
					this.padInner(
						`   ${icon} ${th.fg("accent", `#${t.id}`)} ${th.fg("text", t.title)}  ${th.fg("dim", t.status)}`,
						innerWidth,
					),
				);
			}
		...
		for (const cl of contentLines) {
			lines.push(truncateToWidth(th.fg("borderMuted", "│") + cl + th.fg("borderMuted", "│"), width));
		}

		// ── Footer separator ──
		lines.push(truncateToWidth(th.fg("borderMuted", `├${"─".repeat(innerWidth)}┤`), width));

		// ── Footer shortcuts ──
		const shortcutLine =
			th.fg("muted", "esc") + th.fg("dim", "/") + th.fg("muted", "backspace") + th.fg("dim", ":close");
...
export function registerSprintCommand(pi: ExtensionAPI, getStore: () => TaskStore): void {
		...
		handler: async (args, ctx) => {
			...
			if (!args?.trim()) {
				...
				await ctx.ui.custom<void>(
					(_tui, theme, _kb, done) => new SprintDashboard(active, store, theme, () => done()),
					{ overlay: true },
				);
				return;
			}
			...
			ctx.ui.notify(
				"Use the task tool for sprint management: create_sprint, start_sprint, complete_sprint, assign_sprint, sprint_status, list_sprints",
				"info",
			);
		},
	});
