...
import type { Task, TaskGroup, TaskStore } from "../types.js";
...
export class TaskListOverlay {
	...
	render(width: number): string[] {
		...
		const countParts = Object.entries(counts).map(([s, c]) => `${c} ${s.replace(/_/g, " ")}`);
		contentLines.push(this.padInner(` ${th.fg("dim", countParts.join(" · "))}`, innerWidth));
		contentLines.push(this.padInner("", innerWidth));

		if (this.entries.length === 0) {
			contentLines.push(
				this.padInner(` ${th.fg("dim", "No tasks yet. Ask the agent to create some!")}`, innerWidth),
			);
		} else {
			// Adjust scroll
	...
	private renderGroupHeader(
		entry: Extract<ListEntry, { type: "group-header" }>,
		isSelected: boolean,
		innerWidth: number,
	): string {
		const th = this.theme;
...
// ─── Overlay (exported for shortcut and sprint-command use) ──────
...
export function registerTasksCommand(pi: ExtensionAPI, getStore: () => TaskStore): void {
		...
		handler: async (args, ctx) => {
			...
			if (!ctx.hasUI) {
				...
				const lines = tasks.map(
					(t) => `${STATUS_ICONS[t.status]} #${t.id} [${t.priority}] ${t.title} (${t.status})`,
				);
