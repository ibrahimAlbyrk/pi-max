 ...
 *
 * Groups are organizational containers only — they have no status and cannot be started/completed.
 */

import { createGroup, findGroup, findTask, getGroupTasks, recalculateNextIds } from "../store.js";
...
export function handleDeleteGroup(store: TaskStore, params: TaskActionParams): TaskToolResult {
	...
	const unassignedMsg = groupTasks.length > 0 ? ` (${groupTasks.length} task(s) moved to ungrouped)` : "";
...
export function handleAssignGroup(store: TaskStore, params: TaskActionParams): TaskToolResult {
	...
	const fromStr = oldGroupId !== null ? `G${oldGroupId}` : "ungrouped";
...
export function renderTreeText(store: TaskStore): string[] {
	...
	// Render groups with their tasks and progress "(done/total)"
