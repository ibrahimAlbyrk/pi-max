...
export function planMerge(existing: Task[], incoming: ParsedTask[]): MergePlan {
	...
	for (const inc of incoming) {
		const match = existing.find((e) => e.title.toLowerCase().trim() === inc.title.toLowerCase().trim());
 ...
 *
 * Two-pass approach:
 *   Pass 1 — create all new tasks
 *   Pass 2 — resolve dependsOnTitles → IDs (requires created tasks to be in store)
 */
export function applyMerge(store: TaskStore, plan: MergePlan): MergeResult {
	// Apply updates
	...
	for (const parsed of plan.creates) {
		...
		if (parsed.parentTitle) {
			...
			let group: TaskGroup | undefined = store.groups.find((g) => g.name.toLowerCase().trim() === parentTitle);
			if (!group) {
				// Check if a task with this title exists and could become a group reference
	...
	// Pass 2 — resolve dependsOnTitles → dependsOn IDs
	for (const parsed of plan.creates) {
...
/**
 * Format a merge plan as a human-readable summary string.
 */
