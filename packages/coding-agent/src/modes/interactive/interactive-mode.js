...
export class InteractiveMode {
    ...
    async checkForNewVersion() {
        if (process.env.PI_SKIP_VERSION_CHECK || process.env.PI_OFFLINE)
            return undefined;
        try {
            const response = await fetch("https://registry.npmjs.org/@agentic-indy/pi-max/latest", {
        ...
        switch (message.role) {
            ...
            case "user": {
                ...
                if (textContent) {
                    ...
                    if (skillBlock) {
                    ...
                    else if ("originalCommand" in message &&
                        ...
                        this.chatContainer.addChild(new Spacer(1));
                        const component = new TemplateInvocationMessageComponent(message.originalCommand, textContent, this.getMarkdownThemeWithSettings());
                        component.setExpanded(this.toolOutputExpanded);
                        this.chatContainer.addChild(component);
                    }
                    else {
    ...
    showNewVersionNotification(newVersion) {
        const action = theme.fg("accent", getUpdateInstruction("@agentic-indy/pi-max"));
        const updateInstruction = theme.fg("muted", `New version ${newVersion} is available. `) + action;
        const changelogUrl = theme.fg("accent", "https://github.com/ibrahimAlbyrk/pi-max/blob/main/packages/coding-agent/CHANGELOG.md");
