...
export class TemplateInvocationMessageComponent extends Box {
    ...
    expandedContent;
    markdownTheme;
    constructor(originalCommand, expandedContent, markdownTheme = getMarkdownTheme()) {
    ...
    updateDisplay() {
        ...
        if (this.expanded) {
            ...
            this.addChild(new Markdown(this.expandedContent, 0, 0, this.markdownTheme, {
                color: (text) => theme.fg("customMessageText", text),
            }));
        }
        else {
            // Collapsed: single line - [template] original command (hint to expand)
            const line = theme.fg("customMessageLabel", `\x1b[1m[template]\x1b[22m `) +
...
//# sourceMappingURL=template-invocation-message.js.map