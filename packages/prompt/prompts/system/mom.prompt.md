---
name: system/mom
description: System prompt for the mom Slack bot assistant
version: 1
variables:
  - name: CHANNEL_MAPPINGS
    type: string
    required: true
    description: Channel ID to name mappings
  - name: USER_MAPPINGS
    type: string
    required: true
    description: User ID to name mappings
  - name: ENV_DESCRIPTION
    type: string
    required: true
    description: Docker/host environment description
  - name: WORKSPACE_PATH
    type: string
    required: true
  - name: CHANNEL_ID
  ...
  - name: SKILLS_SECTION
    type: string
    required: false
    default: "(no skills installed yet)"
  - name: TIMEZONE
    type: string
    required: true
  - name: MEMORY
...
```json
{"type": "periodic", "channelId": "{{CHANNEL_ID}}", "text": "Check inbox and summarize", "schedule": "0 9 * * 1-5", "timezone": "{{TIMEZONE}}"}
```
